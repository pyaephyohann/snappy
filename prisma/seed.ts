...
async function main() {
  ...
  // Check if DEFAULT_PASSCODE is set in production
  const isProduction = process.env.NODE_ENV === "production";
  const defaultPasscode = process.env.DEFAULT_PASSCODE;

  if (isProduction && !defaultPasscode) {
    console.error("❌ ERROR: DEFAULT_PASSCODE must be set in production");
    console.error("   Set DEFAULT_PASSCODE environment variable to a secure value");
    process.exit(1);
  }

  const passcodeToUse = defaultPasscode || "snappy123";
  ...
  if (!existingCredential) {
    ...
    if (!isProduction) {
      console.log(`   Default passcode: ${passcodeToUse}`);
    }
  } else {
  ...
  if (!existingAdminCredential) {
    const adminPasscodeHash = await bcrypt.hash(passcodeToUse, 10);
    ...
    if (!isProduction) {
      console.log(`   Default admin passcode: ${passcodeToUse}`);
    }
  } else {
    console.log("⏭️  Admin credential already exists, skipping");
  }

  // Seed Users (only in development, skip in production)
  if (!isProduction) {
    const users = [
      {
        name: "Ikki",
    ...
    // Seed Snaps (only in development)
    const snapData = [
      // Alice's snaps
      {
    ...
    console.log("⏭️  Skipping sample users and snaps in production");
  }
